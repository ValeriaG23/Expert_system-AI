...
def generate_question_tree(known_rules, used_rules, all_rules, current_tourist_type):
    questions = set()  # Folosim un set pentru a ne asigura că întrebările sunt unice
    for rule in all_rules:
        ...
        if question not in known_rules and question not in used_rules:
            questions.add(question)  # Adăugăm întrebarea la set
    return list(questions)  # Convertim înapoi în listă pentru a returna rezultatul
