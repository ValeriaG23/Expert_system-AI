...
def forward_chain(rules, data, apply_only_one=False, verbose=False):
    # Initializează o variabilă pentru a stoca datele anterioare
    old_data = ()

    # Continuă până când setul curent de date coincide cu setul anterior
    while set(old_data) != set(data):
        # Copiază setul curent de date în setul anterior
        old_data = list(data)

        # Parcurge fiecare regulă în listă
        for condition in rules:
            # Aplică condiția regulii pe setul curent de date
            data = condition.apply(data, apply_only_one, verbose)

            # Dacă s-au produs schimbări în date, întrerupe bucla și trece la următoarea iterație
            if set(data) != set(old_data):
                break

    # Returnează setul final de date
    return data


def backward_chain(rules, hypothesis, verbose=False):
    # Inițializăm o variabilă care indică dacă am găsit vreo potrivire între ipoteză și reguli
    anyMatches = False
    # Inițializăm o variabilă pentru rezultatele procesului de backward chaining, inițializată cu ipoteza dată
    results = OR(hypothesis)
    # Parcurgem fiecare regulă în listă
    for rule in rules:
        # Parcurgem fiecare afirmație din partea dreaptă a regulii (consecventa)
        for statement in rule.consequent():
            # Verificăm dacă există o potrivire între afirmația curentă și ipoteză
            matchOutput = match(statement, hypothesis)
            # Dacă există o potrivire, procesăm regulă
            if matchOutput != None:
                anyMatches = True
                # Populăm antecedentul regulii cu vocabularul potrivirii
                populatedAnt = populate(rule.antecedent(), matchOutput)
                # Dacă antecedentul este o listă de afirmații (AND/OR)
                if isinstance(populatedAnt,list):
                    # Inițializăm variabile pentru stocarea rezultatelor AND/OR
                    ands = AND()
                    ors = OR()
                    # Parcurgem fiecare afirmație din antecedentul populat
                    for statement in populatedAnt:
                        # Dacă afirmația este de tip AND, procesăm recursiv folosind backward chaining
                        if isinstance(populatedAnt,AND):
                            ands.append(backward_chain(rules,statement))
                        # Dacă afirmația este de tip OR, procesăm recursiv folosind backward chaining
                        elif isinstance(populatedAnt,OR):
                            ors.append(backward_chain(rules,statement))
                    # Adăugăm rezultatele AND/OR la rezultatele procesului de backward chaining
                    if len(ands)!=0:
                        results.append(ands)
                    elif len(ors)!=0:
                        results.append(ors)
                # Dacă antecedentul nu este o listă de afirmații, procesăm recursiv folosind backward chaining
                else:
                    results.append(backward_chain(rules,populatedAnt))
    # Dacă nu am găsit nicio potrivire, adăugăm ipoteza inițială la rezultatele finale
    if anyMatches == False:
        results.append(OR(hypothesis))
    # Simplificăm rezultatele și le returnăm
